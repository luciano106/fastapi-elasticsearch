...
if ENVIRONMENT == "local":
    @router.post("/token", summary="Generate JWT Token", description="""
    This endpoint generates a **JWT access token** for authentication.

    ### How it works:
    1. In Swagger UI (`/docs`):
    - Click "Authorize" (🔒).
    - Enter your `username` and `password` (e.g., `user` / `password`).
    ...
    - Make a `POST /token` request to retrieve a token.
